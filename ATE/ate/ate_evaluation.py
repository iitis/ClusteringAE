...
def evaluate_autoencoder(X,
    ...
    ----------
        ...
        rpath: folder name for saving results (or None)
        vpath: folder name for saving visualisations (or None)
        fname: file name for saving results (or None)
    ...
    keys = ['reconstruction_error_RMSE_multiplication',
    ...
    keys = ['reconstructed_simplex_dot_product',
            'sorted_abundances',
            'sorted_endmembers']
    output_method = dict.fromkeys(keys)

    # Calculate endmember errors and select proper permutation
    results['endmembers_error'], order = best_permutation(endmembers,
                                                          endmembers_gt,
                                                          SAD_distance)
    ...
    results['volume_simplex_dot_product'] = volume_simplex(
        torch.from_numpy(endmembers)).item()
    ...
    abundances_gt = repair_order_of_pixels(abundances_gt,
                                           image_shape,
                                           fname)
    ...
    if fname is not None and fname != "":
        np.savez(os.path.join(rpath, fname), **results)
        if vpath is not None and vpath != "":
            compare_endmembers(endmembers,
                               endmembers_gt,
                               folder=vpath,
                               name=fname)
...
def repair_order_of_pixels(array, original_shape, name):
    """
    In some images the order of abundances is wrong.
    This function repair this error.

    Arguments:
    ----------
       array - 2D abundance numpy array (n_samples, n_endmembers)
       original_shape - tuple with width, height and n_endmembers
                        for the array
       name - name of the image
    """
    #TODO: Prepare unittest
    if 'Samson' in name:
        cube = array.reshape((original_shape[0],
                              original_shape[1],
                              array.shape[-1]))
        cube = np.transpose(cube, (1, 0, 2))
        return np.reshape(cube, (-1, cube.shape[-1]))
...
def mean_RMSE_error(X, Y):
    ...
    '''
    check_arrays_shapes(X, Y)
    #TODO: Check calculations according to patterns

    RMSE_errors = [mean_squared_error(X[i], Y[i], squared=False) \
...
def investigate_simplex(points, vertices, folder='./Test', filename=None):
    ...
    if dimension + 1 > no_of_vertices:
        print('Transformation from data space to simplex space.')
        x0, Pv, vertices = decompose(vertices)
        points = to_subspace(x0, Pv, points)

    for i in range(points.shape[0]):
        barycentric_coordinate = get_barycentric_coordinate(points[i],
                                                            vertices)
        check_if_inside_simplex = check_point_in_simplex(
            barycentric_coordinate)
        assignment[i] = check_if_inside_simplex

    number_of_points_inside_simplex = np.count_nonzero(assignment)
    percentage = 100. * number_of_points_inside_simplex / assignment.shape[0]
    # Prepare plot
    draw_points_with_simplex(points,
                             vertices,
                             assignment,
                             percentage,
                             folder,
