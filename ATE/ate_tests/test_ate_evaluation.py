...
from ate.ate_evaluation import (mean_RMSE_error, convert_to_numpy, 
    SAD_distance, evaluate_autoencoder, investigate_simplex, 
    draw_points_with_simplex, get_barycentric_coordinate, 
    check_point_in_simplex, decompose, to_subspace, best_permutation, 
    change_order_horizontally, change_order_vertically, check_arrays_shapes)


class Test(unittest.TestCase):
    def test_mean_RMSE_error(self):
        a1 = np.array([[0., 1., 2.], [3., 4., 0.]])
        b1 = np.array([[1., 2., 0.], [1., 0., 2.]])
    ...
    def test_convert_to_numpy(self):
        ...
        list_1 = [1, 2, 3]
        res_tens_1, res_array_1, res_list_1 = map(convert_to_numpy,
                                                 (tens_1, array_1, list_1))
    ...
    def test_evaluate_autoencoder(self):
        X = torch.Tensor([[1.0, 2.0, 0.0, 1.0],
                          [0.0, 1.0, 5.0, 2.5],
                          [0.5, 0.0, 2.0, 1.0]])
        abundances_gt = torch.Tensor([[0.0, 0.4, 0.6],
                                      [0.8, 0.1, 0.1],
                                      [1.0, 0.0, 0.0]])
        endmembers_gt = torch.Tensor([[0.1, 0.5, 0.8, 1.0],
                                      [1.0, 2.0, 0.0, 3.0],
                                      [0.5, 1.0, 2.0, 3.0]])
        # Change the order of rows: proper choice -> [1, 2, 0]
        endmembers = np.array([[0.5, 1.0, 2.0, 3.0],
                               [0.2, 0.7, 0.8, 1.0],
                               [0.8, 2.2, 0.2, 2.8]])
        # Change the order of columns: proper choice -> [1, 2, 0]
        abundances = np.array([[0.7, 0.1, 0.3],
                               [0.4, 0.6, 0.0],
                               [0.0, 1.0, 0.0]])
        result, _ = evaluate_autoencoder(X, abundances, endmembers,
                                         abundances_gt, endmembers_gt)
        print(f'reconstruction_error_RMSE_multiplication: {result["reconstruction_error_RMSE_multiplication"]}')
        print(f'abundances_error_multiplication: {result["abundances_error_multiplication"]}')
        print(f'endmembers_error: {result["endmembers_error"]}')
        self.assertTrue(np.abs(result['reconstruction_error_RMSE_multiplication'] - 1.3119) < 0.001)
        self.assertTrue(np.abs(result['abundances_error_multiplication'] - 0.10534) < 0.001)
    ...
    def test_investigate_simplex(self):
        ...
        for endmembers, name in zip(vertices_list, names):
            _, _ = investigate_simplex(points,
                                       endmembers,
                                       folder=folder_for_saving,
                                       filename=name)
        
        # Synthetic data
        test_vertices_1 = np.array([[-1., 1.],
                                  [1., -2.],
                                  [3., 3.]])
        # ---- TEST 1 ------
        # all points from the following set are inside simplex
        points_test_1 = np.array([[3., 3.],
                                  [1., 1.],
                                  [0, 0.5],
                                  [1.2, -0.8]])
        _, no_of_points = investigate_simplex(points_test_1,
                                              test_vertices_1,
                                              folder=folder_for_saving,
                                              filename='test_1')
        ...
        # ---- TEST 2 ------
        points_test_2 = np.array([[3., 1.],
                                  [-1, 1],
                                  [-1., -1.],
                                  [1., -2.],
                                  [0., 2.]])
        _, no_of_points = investigate_simplex(points_test_2,
                                              test_vertices_1,
                                              folder=folder_for_saving,
                                              filename='test_2')
        self.assertTrue(np.allclose(
        ...
        test_vertices_2 = np.array([[0., 0., 1.], [2., 3., 0.], [2., 3., 1.]])
        points_test_3 = np.array([[1.7, 51/20, 0.3], [2., 3., 1.5],
                                  [1.5, 3., 0.6], [1.5, 3., 0.],
                                  [4/3, 2., 2/3]])
        assignment, no_of_points = investigate_simplex(points_test_3,
                                                       test_vertices_2,
                                                       folder=folder_for_saving,
                                                       filename='test_3')
        ...
        # ---- TEST 4 ------
        # dimension + 1 > number of simplex vertices
        test_vertices_3 = np.array([[0, 9, 0, 3, 3.5], [7, 8, 0, 6, 1],
                                    [4, 10, -2, 6, 6], [2, 5, -1, 3, 3]])
        points_test_4 = np.array([[1, 2, 3, 4, 5], [13/4, 8, -3/4, 9/2, 3.375],
                                  [3.3, 9.05, -0.95, 4.95, 4.1], [10, -3.5, 7, 0, 0],
                                  [7.1, 8.4, -1.1, 6.6, 2.95]])
        assignment, no_of_points = investigate_simplex(points_test_4,
                                                       test_vertices_3,
                                                       folder=folder_for_saving,
                                                       filename='test_4')
        self.assertTrue(np.all(
            assignment == np.array([False, True, True, False, False])
        ))
        self.assertTrue(np.allclose(
            no_of_points, 40.0
            ))
    ...
    def test_get_barycentric_coordinate(self):
        ...
        self.assertTrue(np.allclose(
            result, np.array([18/17, -10/17, 9/17])
            ))
        ...
        self.assertTrue(np.allclose(
            result_2, np.array([0.04, 0.56, 0.4])
            ))
        ...
        self.assertTrue(np.allclose(
            result_3, np.array([6/7, -2/7, 3/7])
            ))

        # 4-simplex
        # ---- TEST 4 -------
        point_4 = np.array([1, 2, 3, 4])
        vertices_4 = np.array([[2, 5, -1, 3], [5, 3, 1, 3], [-2, 4, 1, 0],
                               [4, 3, -1, 1], [1, 2, 1, 3]])
    ...
    def test_decompose(self):
        M = np.array([[2, 5, -1, 3], [5, 3, 1, 3], [-2, 4, 1, 0],
                      [4, 3, -1, 1], [1, 2, 1, 3]])
        x0, Pv, Mp = decompose(M)
        self.assertTrue(np.all((
            x0 == np.array([2, 17/5, 1/5, 2])
        )))
        Pv_true = np.array([[-0.940561, 0.100236, 0.123149, -0.300221],
                            [-0.0935564, -0.655624, 0.664245, 0.346675],
                            [0.243029, -0.420089, 0.0742667, -0.871175]])
        for i, v in enumerate(Pv):
            self.assertTrue(
                np.allclose(v, Pv_true[i]) or np.allclose(-v, Pv_true[i])
            )
        Mp_true = np.array([[-0.2876222, -1.4994174, -1.63243744],
    ...
    def test_to_subspace(self):
        x0 = np.array([2, 17/5, 1/5, 2])
        Pv = np.array([[0.940561, -0.100236, -0.123149, 0.300221],
                       [-0.0935564, -0.655624, 0.664245, 0.346675],
                       [0.243029, -0.420089, 0.0742667, -0.871175]])
        X = np.array([[1, -0.5, 0.6, 1.3],
                      [0, 1, 1.2, 7]])
        Xp = to_subspace(x0, Pv, X)
        self.assertTrue(
            np.allclose(Xp, np.array([[-0.809055, 2.67352, 2.03485],
                                      [-0.2626, 4.15823, -3.75945]]))
        )
    ...
    def test_check_arrays_shapes(self):
        check_arrays_shapes(np.array([[1, 0], [1, 2], [1, 4]]),
