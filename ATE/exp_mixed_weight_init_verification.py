...
def set_experiment_name(params_global,
                        additional_name):
    '''
...
def run_mixed_algorithm(number, weight):
    ...
    parameters['global']['experiment_name'] = set_experiment_name(
        parameters['global'],
        additional_name=f'{number}_verification'
    )

    dataset = get_dataset(parameters['global']['dataset_name'],
    ...
    if i == 0 and j == 0:
        initialize_new_models(parameters['global']['number_of_models'],
    ...
    try:
        results, estimated_abundances, estimated_endmembers, percentage, \
            trained_model, estimated_simplex = experiment_simple(
                parameters['global']['autoencoder_name'],
        ...
        save_model(f'{fullname}/model_{i}/{j}_training',
                   trained_model)
        save_output_autoencoder(estimated_simplex,
        ...
        current_run_result = {
            ...
            'reconstruction_error_MSE': results['reconstruction_error_MSE_multiplication'],
            'reconstruction_error_RMSE': results['reconstruction_error_RMSE_multiplication'],
...
def initialize_new_models(number_of_models,
                          autoencoder_name,
                          params_aa,
                          params_global,
                          path):
    ...
    for i in range(number_of_models):
        ...
        net = get_autoencoder(autoencoder_name,
                              params_aa,
                              params_global)
...
def test_designate_number_of_experiment():
    ...
    assert (1, 1, 4) == designate_number_of_experiment(
        os.getcwd(),
        test_filename_1,
        3
    )
    ...
    assert (2, 0, 6) == designate_number_of_experiment(
        os.getcwd(),
        test_filename_2,
        3
    )
