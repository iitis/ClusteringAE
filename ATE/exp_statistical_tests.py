...
def select_data(number_of_models,
                number_of_runs,
                models_idx,
                table,
                variable):
...
def prepare_post_hoc_test(data,
                          number,
                          weight_method,
                          day):
    '''
    ...
    post_hoc_test.to_csv(f'{number}_{weight_method}_{day}_post_hoc_test_table.csv',
                         index=False, header=False)
    sp.sign_plot(post_hoc_test, **heatmap_args)
    plt.title(f"Post-hoc analysis of Conover-Iman test for {number}_{weight_method}_{day}",
...
def prepare_statistical_tests(experiment_ids,
                              weights_methods,
                              experiment_days,
                              experiment_data,
                              variable):
    ...
    for weight_method, number, day in product(weights_methods,
                                              experiments_ids,
                                              experiment_days):
        ...
        if os.path.isfile(filename):
            ...
            results = select_data(number_of_models,
                                  number_of_runs,
                                  models_idx,
                                  table,
                                  variable)
            ...
            test_results_for_experiments.append(
                tests_single_result
            )
...
def unittest_select_data():
    ...
    test_dataframe = pd.DataFrame(test_table,
                                  columns=['model', 'run', 'value'])
    models_idx = test_dataframe['model'].unique()
    proper_models_idx = np.array([0, 10])
    assert (models_idx == proper_models_idx).all()
    proper_result = np.array([
        [0.341, 1.241, -1.3587],
        [5.3789, 2.8794, 4.7954]
    ])
    result = select_data(number_of_models=2,
                         number_of_runs=3,
...
if __name__ == "__main__":
    ...
    location = '/run/user/1000/gvfs/smb-share:server=192.168.100.6,share=projects/Actual/autoencoders_unmixing/results/weights_initialization/final_experiments/Finished/Detailed_results/'
    os.chdir(location)

    experiments_ids = ['F001', 'F002', 'F003', 'F004', 'F005',
                       'F006', 'F007', 'F008', 'F009', 'F010',
                       'F011']
    weights_methods = ['XGU', 'XGN', 'KHN', 'KHU']
    experiment_days = ['04022021']
    experiment_data = {
    ...
    results = prepare_statistical_tests(experiments_ids,
                                        weights_methods,
                                        experiment_days,
                                        experiment_data,
                                        variable)
