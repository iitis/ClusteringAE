...
for dname in Custom
do
    for grid_shape in '(5,5)'
    do
        i=0
        for weights_init in Kaiming_He_normal
        do
            for model_no in 0
            do
                python3 -m grids.exp --exp-name $dname'_'$grid_shape --run-name $i --dname $dname --model-no $model_no --grid-shape $grid_shape --weights-init $weights_init --seed $i
                i=$((i+1))
            done
        done
    done
